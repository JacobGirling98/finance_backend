...
class StandingOrderProcessorTest : FunSpec({
    ...
    test("will process weekly standing order if current date is after standing order date") {
        ...
        val expected =
            Factory(date = Date(weekBeforeNow.plusWeeks(1)), frequency = Frequency.WEEKLY, id = uuid).standingOrder()
        ...
        standingOrdersDatabase.ordersUpdated
            ...
            .shouldContain(
                firstFactory.standingOrder()
                    .copy(
                        nextDate = Date(
                            firstFactory.standingOrder().nextDate.value.plusMonths(
                                1
                            )
                        )
                    )
            ...
            .shouldContain(
                secondFactory.standingOrder()
                    .copy(
                        nextDate = Date(
                            secondFactory.standingOrder().nextDate.value.plusWeeks(
                                1
                            )
                        )
                    )
